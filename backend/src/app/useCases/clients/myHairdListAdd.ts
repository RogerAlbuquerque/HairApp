...
export async function myHairdList(req: Request, res:Response){
  ...
  const hairdExist = await Hairdresser.findOne({ hairdName: hairdName});

  if(hairdExist){
    await Client.findByIdAndUpdate(req.params.clientId, {hairdressers:hairdExist._id}).populate({path:'hairdressers',
